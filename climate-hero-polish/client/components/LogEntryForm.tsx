...
export function LogEntryForm() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      // A new entry changes today's total, the activity list, and which
      // insights are relevant — all three views need to refetch.
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-medium text-muted-foreground" id={`${quantityInputId}-activity-label`}>
          Activity
        </label>
        <Select value={activityId} onValueChange={setActivityId}>
          <SelectTrigger aria-labelledby={`${quantityInputId}-activity-label`}>
      ...
      <div className="flex flex-col gap-1.5">
        <label htmlFor={quantityInputId} className="text-sm font-medium text-muted-foreground">
        ...
        <Input
          ...
          aria-describedby={selected?.hint ? `${quantityInputId}-hint` : undefined}
        />
        {selected?.hint && (
          <p id={`${quantityInputId}-hint`} className="text-xs text-muted-foreground">
            {selected.hint}
          </p>
        )}
      </div>
      ...
      <div className="flex items-center justify-between rounded-xl border border-dashed border-border px-3 py-2">
        ...
        <span
          className="font-mono-data text-sm font-semibold text-primary"
          aria-live="polite"
        >
