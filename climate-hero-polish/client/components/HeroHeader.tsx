...
export function HeroHeader() {
  ...
  const progressPct = Math.min((level.pointsIntoLevel / level.pointsForNextLevel) * 100, 100);
    ...
    <div className="flex flex-col items-center gap-2 text-center">
      ...
      <div className="mt-3 flex w-full max-w-xs flex-col gap-1.5">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-mono-data text-muted-foreground">{totalPoints} pts</span>
        </div>
        <div
          className="h-1.5 w-full overflow-hidden rounded-full bg-muted"
          role="progressbar"
          aria-label={`Progress toward level ${level.level + 1}`}
          aria-valuenow={level.pointsIntoLevel}
          aria-valuemin={0}
          aria-valuemax={level.pointsForNextLevel}
        >
          <div
